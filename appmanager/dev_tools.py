...
"""
    ...
    def compile_requirements(self) -> bool:
        ...
        # Ensure pip-tools is installed and up-to-date
        if not utils.is_package_installed("pip-tools"):
            print("-> pip-tools not found. Installing...")
            utils.install_packages(["pip-tools"])
        else:
            # Update pip-tools to ensure compatibility with current pip version
            print("-> Ensuring pip-tools is up-to-date...")
            try:
                utils.spin_start("   ✅ Updating pip-tools")
                subprocess.run([sys.executable, "-m", "pip", "install", "--upgrade", "pip-tools", "--quiet"], 
                             check=True, capture_output=True)
                utils.spin_stop("   ✅ pip-tools is ready")
            except subprocess.CalledProcessError:
                print("   ⚠️ Could not update pip-tools, but will try to continue")
    ...
    def _compile_single_file(self, in_file, out_file) -> bool:
        ...
        try:
            command = [
                ...
                "--no-header",  # Remove the header comment block
                "--no-annotate",  # Remove source annotations that contain absolute paths
